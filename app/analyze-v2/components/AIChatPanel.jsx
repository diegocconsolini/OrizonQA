...
export default function AIChatPanel({
  ...
  // Flow state: 'idle' | 'source-selected' | 'configuring' | 'analyzing' | 'complete' | 'error'
  ...
  // Sync flowState with source selection
  useEffect(() => {
    ...
    // Source was cleared
    if (!hasSource && flowState !== 'idle' && flowState !== 'analyzing' && flowState !== 'complete') {
      setFlowState('idle');
      setSelectedAction(null);
    }
    // Source was selected (new or different)
    else if (hasSource && prevSourceRef.current !== currentSourceKey && flowState === 'idle') {
      setFlowState('source-selected');
    }

    prevSourceRef.current = currentSourceKey;
  }, [selectedFiles, codeInput, uploadedFiles, flowState]);

  // Sync flowState with analysis status
  useEffect(() => {
    if (isAnalyzing && flowState !== 'analyzing') {
      setFlowState('analyzing');
    }
    if (isComplete && flowState !== 'complete') {
      setFlowState('complete');
    }
    if (status === AnalysisStatus.ERROR && flowState !== 'error') {
      setFlowState('error');
    }
  }, [isAnalyzing, isComplete, status, flowState]);

  // When action is selected, move to configuring
  useEffect(() => {
    if (selectedAction && flowState === 'source-selected') {
      setFlowState('configuring');
    }
  }, [selectedAction, flowState]);
  ...
  const handleSendChat = useCallback(async () => {
    ...
    try {
      ...
      // Add assistant response (AI may suggest actions but user must click)
      setChatMessages(prev => [...prev, {
        type: 'assistant',
        content: data.response,
        model: MODELS.chat.name,
        tokens: data.usage,
        suggestedAction: data.suggestedAction // Store but don't auto-trigger
      }]);
  ...
  return (
    <div className="h-full flex flex-col bg-surface-dark/30">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {/* Welcome Message - IDLE state */}
        {flowState === 'idle' && (
          <ChatMessage type="assistant" model={MODELS.chat.name}>
            <div className="space-y-3">
              <p className="text-sm text-white leading-relaxed">
                Welcome! Select code from the left panel to begin analysis.
              </p>
              ...
              <div className="p-2 bg-white/5 rounded-lg border border-white/10">
                <p className="text-[10px] text-text-secondary-dark flex items-center gap-2">
                  ...
                  <span>
                    <strong className="text-cyan-400">{MODELS.chat.name}</strong> for chat •{' '}
              ...
              <div className="space-y-2">
                <StepItem num={1} title="Select source" desc="GitHub, paste, or upload" status="current" />
                <StepItem num={2} title="Choose output" desc="Tests, stories, criteria" status="pending" />
                <StepItem num={3} title="Generate" desc="AI analyzes your code" status="pending" />
              </div>
            </div>
          </ChatMessage>
        )}

        {/* Source Selected - show action buttons */}
        {flowState === 'source-selected' && source && (
          <ChatMessage type="assistant" model={MODELS.chat.name}>
            <div className="space-y-3">
              <p className="text-sm text-white leading-relaxed">
                I see <span className="text-primary font-medium">{source.label}</span> selected.
              ...
              <div className="space-y-2 mt-2">
                <StepItem num={1} title="Select source" desc={source.label} status="complete" />
                <StepItem num={2} title="Choose output" desc="Click an option above" status="current" />
                <StepItem num={3} title="Generate" desc="AI analyzes your code" status="pending" />
              </div>
            </div>
          </ChatMessage>
        ...
        {/* Configuring - action selected, ready to generate */}
        {flowState === 'configuring' && selectedAction && source && (
