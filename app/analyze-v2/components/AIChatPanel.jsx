 ...
 * Features conversational flow, streaming output, and result actions.
 */
  ...
  return (
    <div className="h-full flex flex-col bg-surface-dark/30">
      ...
      <div className="flex-shrink-0 px-4 py-2.5 border-b border-white/10 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-green-400">
            {hasApiKey ? `Connected - ${claudeModel?.replace('claude-', '').replace('-20250514', '')}` : 'API key required'}
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {selectedAction && status === AnalysisStatus.IDLE && (
          <>
                  ...
                  {viewMode === 'simple' ? (
                    ...
                    <div className="space-y-1.5 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white">Estimated cost: ~$0.05</span>
                      </div>
                    </div>
                  )}
                </div>
      ...
      {/* Chat Input (for future chat functionality) */}
      <div className="flex-shrink-0 p-3 border-t border-white/10">
        <div className="relative">
          <input
            type="text"
            placeholder="Ask me anything or select an action above..."
          ...
          <button
            disabled
            className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-primary/50 rounded"
          >
            <Send className="w-3.5 h-3.5 text-white" />
          </button>
        </div>
        <div className="flex items-center justify-between mt-1.5 text-[10px] text-text-secondary-dark">
          <span>{provider === 'claude' ? claudeModel?.replace('claude-', '') : 'LM Studio'}</span>
          <div className="flex items-center gap-2">
            <kbd className="px-1 py-0.5 bg-white/5 rounded">Cmd+Enter</kbd>
