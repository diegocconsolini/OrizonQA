...
| **Core Differentiator** | **Git Integration** | Our strongest feature - must be first-class experience |

---

## 🌟 CORE DIFFERENTIATOR: Git Integration

**Git integration is ORIZON's strongest competitive advantage** and must be treated as a first-class citizen throughout the product.

### Current Git Capabilities (Already Built)

We have a **solid foundation** with existing OAuth infrastructure:

| Feature | Status | Location |
|---------|--------|----------|
| GitHub OAuth Flow | ✅ Complete | `/lib/oauth/adapters/GitHubAdapter.js` |
| User Authorization | ✅ Complete | `/api/oauth/github/connect/route.js` |
| Token Management | ✅ Complete | `/lib/oauth/encryption.js` |
...
### Git Integration Vision: "Repository-First Analysis"

**Transform ORIZON from "paste code" to "connect your repository"**

The ideal user journey:
1. Connect GitHub account (one-time)
2. Select repository from dropdown
3. Pick branch + files/folders
4. Configure analysis
5. Run analysis on actual production code
6. Results linked to specific commits/files

### Advanced Git Features to Build

#### Phase 1: Enhanced Repository Browser

**Repository Selector Component:**
```jsx
<RepositorySelector
  onSelect={(repo) => setSelectedRepo(repo)}
  showPrivate={hasRepoScope}
  sortBy="updated"
  searchable={true}
  favorites={userFavoriteRepos}
/>
```

**Features:**
- Visual repository list with icons (private/public)
- Search/filter repositories
- Star/favorite repos for quick access
- Show recent activity (last commit date)
- Repository stats (stars, language, size)
...
- Tags support
- Compare branches (run analysis on diff)
- PR branch detection

**Commit-Linked Analysis:**
```jsx
<CommitSelector
  repo={selectedRepo}
  branch={branch}
...
- Link analysis results to commit SHA
- Show commit message and author
- "Analyze this PR" functionality
...
<PRAnalyzer
  repo={selectedRepo}
  prNumber={prNumber}
  onAnalyze={handlePRAnalysis}
/>
```

**Features:**
- List open PRs for repository
- Analyze PR diff (changed files only)
- Auto-generate test suggestions for new code
- Post analysis results as PR comment
- "Request Tests" GitHub Action integration

**PR Comment Integration:**
```markdown
## 🔬 ORIZON QA Analysis

**Analyzed Files:** 12 files changed in this PR

### Suggested Tests
- `UserAuth.login()` - 5 test cases recommended
- `PaymentController.process()` - 8 test cases (security focus)

### Coverage Recommendations
- Missing edge cases for null input handling
- Security tests recommended for auth endpoints

[View Full Analysis](https://orizon-qa.vercel.app/analyze/abc123)

---
*Generated by ORIZON QA Analyzer*
```
...
<RepositoryWatch
  repo={selectedRepo}
  schedule="on_push" // or "daily", "weekly"
  branches={["main", "develop"]}
  notifyOn={["coverage_drop", "security_issue"]}
/>
```
...
- Badge generation (like CI badges)

**GitHub Action:**
```yaml
# .github/workflows/orizon-qa.yml
name: ORIZON QA Analysis
on:
  pull_request:
...
jobs:
  analyze:
    ...
    steps:
      ...
      - uses: orizon-qa/analyze-action@v1
        with:
...
#### Phase 5: Multi-Provider Git Support
...
### Git Integration Database Schema

```sql
-- Linked repositories (for quick access)
CREATE TABLE user_repositories (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  provider VARCHAR(50) NOT NULL DEFAULT 'github',
  connection_id INTEGER REFERENCES oauth_connections(id),
  repo_id VARCHAR(255) NOT NULL,          -- Provider's repo ID
...
-- Repository analysis history (linked to commits)
CREATE TABLE repository_analyses (
  ...
  branch VARCHAR(100),
  pr_number INTEGER,
  files_analyzed TEXT[],                   -- Array of file paths
  created_at TIMESTAMP DEFAULT NOW()
);

-- Scheduled/watched repositories
CREATE TABLE repository_watches (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  repository_id INTEGER REFERENCES user_repositories(id),
  schedule VARCHAR(50),                    -- 'on_push', 'daily', 'weekly'
...
-- PR analysis results (for comment posting)
CREATE TABLE pr_analyses (
  ...
  comment_id VARCHAR(100),                 -- GitHub comment ID
  created_at TIMESTAMP DEFAULT NOW()
);
```

### Git Integration UI Mockups

**Repository Input Section (Redesigned):**
```
┌─────────────────────────────────────────────────────────────────┐
│  📂 Select from your repositories                               │
├─────────────────────────────────────────────────────────────────┤
│  🔍 Search repositories...                    [⭐ Favorites ▾] │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ 🔒 mycompany/backend-api          ★           3 days ago │   │
│  │    JavaScript • 245 files • main                         │   │
│  ├─────────────────────────────────────────────────────────┤   │
│  │ 🌐 mycompany/frontend-app                    1 week ago  │   │
│  │    TypeScript • 180 files • main                         │   │
│  ├─────────────────────────────────────────────────────────┤   │
│  │ 🔒 personal/side-project           ★         2 weeks ago │   │
│  │    Python • 45 files • develop                           │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
...
┌─────────────────────────────────────────────────────────────────┐
│  🌿 Branch: [main ▾]    📝 Commit: [Latest ▾]                  │
├─────────────────────────────────────────────────────────────────┤
│  📁 Select files to analyze:                                    │
│                                                                 │
│  ☑ src/                                                         │
│    ☑ controllers/                                               │
│      ☑ UserController.js (245 lines)                           │
│      ☑ AuthController.js (312 lines)                           │
│      ☐ AdminController.js (189 lines)                          │
│    ☑ services/                                                  │
│      ☑ UserService.js (178 lines)                              │
│    ☐ utils/ (12 files)                                         │
│  ☐ tests/ (already has tests)                                  │
│                                                                 │
│  Selected: 4 files (735 lines) • Est. tokens: ~3,500           │
│                                                                 │
│  [Select All Code Files]  [Select by Pattern: src/**/*.js]     │
└─────────────────────────────────────────────────────────────────┘
```
...
│  Open Pull Requests:                                            │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ #142 Add user authentication                   +245 -12  │   │
│  │      feature/auth → main • 3 files changed               │   │
│  │      [Analyze PR Changes]                                │   │
│  ├─────────────────────────────────────────────────────────┤   │
│  │ #139 Fix payment processing bug                +23 -8    │   │
│  │      fix/payment → main • 2 files changed                │   │
│  │      [Analyze PR Changes]                                │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ☑ Post analysis results as PR comment                         │
...
POST   /api/github/repositories/:id/favorite // Toggle favorite
GET    /api/github/repositories/:id/branches // Get branches
GET    /api/github/repositories/:id/tree     // Get file tree
GET    /api/github/repositories/:id/content  // Get file content

// PR integration
GET    /api/github/repositories/:id/pulls    // List open PRs
GET    /api/github/repositories/:id/pulls/:pr/files  // Get PR changed files
POST   /api/github/repositories/:id/pulls/:pr/analyze  // Analyze PR
POST   /api/github/repositories/:id/pulls/:pr/comment  // Post comment

// Commit analysis
GET    /api/github/repositories/:id/commits  // List recent commits
POST   /api/github/repositories/:id/commits/:sha/analyze  // Analyze commit

// Scheduled analysis
POST   /api/github/repositories/:id/watch    // Set up watch
DELETE /api/github/repositories/:id/watch    // Remove watch
GET    /api/github/watches                   // List user's watches

// Webhooks (for GitHub App)
POST   /api/webhooks/github                  // GitHub webhook handler
```

### Git Integration Competitive Advantage

| Feature | ORIZON | Diffblue | Testim | Snyk |
|---------|--------|----------|--------|------|
| GitHub OAuth | ✅ | ❌ | ✅ | ✅ |
| Private Repos | ✅ | ❌ | ✅ | ✅ |
| Branch Selection | ✅ | ❌ | ❌ | ✅ |
| PR Analysis | 🔄 Building | ❌ | ❌ | ✅ |
| Commit-linked Results | 🔄 Building | ❌ | ❌ | ✅ |
| PR Comments | 🔄 Building | ❌ | ❌ | ✅ |
| Scheduled Analysis | 📋 Planned | ❌ | ❌ | ✅ |
| GitHub Action | 📋 Planned | ✅ | ❌ | ✅ |
| Multi-file Selection | ✅ | ❌ | ❌ | N/A |
| GitLab Support | 📋 Planned | ❌ | ✅ | ✅ |
| Bitbucket Support | 📋 Planned | ❌ | ❌ | ✅ |

**Our Unique Git Advantage:**
- **Repository-first UX** (not URL-based like competitors)
- **Smart file selection** (not "analyze everything")
- **Visual file browser** (not just API access)
- **Commit-linked results** (traceability)
- **PR workflow integration** (suggest tests for new code)
