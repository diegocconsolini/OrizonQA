...
// Dynamically import database modules to avoid build-time errors
...
export async function query(text, params = []) {
  // Load modules dynamically to avoid build-time errors
  await loadModules();

  // Always use pg Pool for consistent parameterized query support
  // @vercel/postgres sql template doesn't support parameterized queries well
  if (!pool) {
    pool = new Pool({
      connectionString: process.env.POSTGRES_URL || 'postgresql://postgres:postgres@localhost:5432/orizonqa',
      max: 10,
