...
export async function query(text, params = []) {
  // For Vercel, use tagged template literal approach
  if (process.env.VERCEL) {
    // Convert parameterized query to tagged template for @vercel/postgres
    // This is a workaround since sql uses template literals
    const result = await sql.query(text, params);
    return result;
  }

  // For local dev, use pg Pool directly
  if (!pool) {
    pool = new Pool({
      connectionString: process.env.POSTGRES_URL || 'postgresql://postgres:postgres@localhost:5432/orizonqa',
      max: 10,
